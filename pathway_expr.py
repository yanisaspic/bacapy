...
def fill_na(path_expr, tp = 17):
    """
    If a pathway does not contain any expression, it is given a list
    filled with '0'.

    Paramètres
    ----------
    graph : tlp.Graph
    tp : int, number of timestamp
...
def get_aggregated_expr_pathway(graph, tp = 17, method="mean"):
    """
    Create a dictionary containing all the expression data of
    the reactions of each pathway, and aggregate them
    
    Paramètres
    ----------
    graph : tlp.Graph
    method : string from : ['mean', 'maxStd', 'minStd', 'upDownZ', 'normalZ']
